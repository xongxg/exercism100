...
pub fn anagrams_for<'a>(word: &str, possible_anagrams: &[&'a str]) -> HashSet<&'a str> {
    let mut low_target = normalize(word);

    possible_anagrams
        .iter()
        .filter(|&w| {
            let mut w_target = normalize(w);
            if w_target.ne(&low_target) {
                w_target.sort();
                low_target.sort();

                return w_target.eq(&low_target);
            }
...
fn normalize(word: &str) -> Vec<char> {
    word.to_lowercase().chars().collect::<Vec<char>>()
}
